...
io.on("connection", function(socket) {
  ...
  socket.on("join", function(joinRequest) {
    socket.join("room-temp-" + joinRequest.tempCode);
    let current_game = updateGame(joinRequest);
    if (current_game) {
      socket.leave("room-temp-" + joinRequest.tempCode);
      socket.join("room-" + joinRequest.code);
      io.sockets.in("room-" + joinRequest.code).emit("startGame", current_game);
    } else {
      io.sockets
        .in("room-temp-" + joinRequest.tempCode)
        .emit("errors", "Sorry game started or invalid room.");
    }
  });

  socket.on("result", function(resultRequest) {
    let endData = {
      username: resultRequest.username,
...
function updateGame(joinData) {
  ...
  if (joinData.code != "") {
    ...
    let obj = games.find((o, i) => {
      if (o.code == joinData.code && o.started == false) {
